...
#common_gene_ids<- as.data.frame(common_gene_ids)
```



## I have not saved these 18 candidate genes wch are the differences between two CID groupd under the drought conditions and save them 


```{r, include=TRUE}
final_candidates_qtl_info <- qtl_data[qtl_data$gene_id%in%common_gene_ids, ]
final_candidate_dge_info <- deg_data[deg_data$gene_id%in%common_gene_ids, ]
final_candidate_wgcna_info <- wgcna_data[ wgcna_data$gene_id%in%common_gene_ids, ]


final_candidate_18 <- final_candidates_qtl_info %>%
  full_join(final_candidate_dge_info , by = "gene_id") %>%
  full_join(final_candidate_wgcna_info, by = "gene_id")


write.table(final_candidate_18, "final_18_candidates_full_info.csv", sep=",", row.names = FALSE)
```
